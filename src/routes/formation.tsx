...
import { club, formationCategories, images, players, positions } from "@/data/club";
import { cn } from "@/lib/utils";

export const Route = createFileRoute("/formation")({
  head: () => ({
    meta: [
      { title: `Formation — ${club.name} (${club.short})` },
      {
        name: "description",
        content: `Le parcours de formation du ${club.short} : de l'École de Basket aux U20, à ${club.city}.`,
      },
      { property: "og:title", content: `Formation — ${club.name}` },
      {
        property: "og:description",
        content: `Les catégories jeunes du ${club.short}, de l'École de Basket aux U20.`,
      },
      { property: "og:url", content: "/formation" },
    ],
    links: [{ rel: "canonical", href: "/formation" }],
  }),
  component: Formation,
});

function Formation() {
  const [category, setCategory] = useState<string>("U20");
  ...
  return (
    <>
      <PageHero
        ...
        title="Formation"
        intro="De l'École de Basket aux U20 : le parcours qui construit les équipes de demain."
        image={images.teamJeunes}
        imageAlt={`Jeunes joueurs du ${club.name} à l'entraînement`}
      />
